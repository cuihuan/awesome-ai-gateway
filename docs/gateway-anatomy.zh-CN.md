...
五处分歧,每一处都读自 2026-07-29 锁定的 commit 源码。这些就是该拿去问厂商的问题,因为它们没有一条会出现在功能对照表里。
