...
5. **Test the statelessness claim in §7 against the products.** Point a 2026-07-28-era client at a gateway advertising session-aware routing and watch whether it mints or echoes `Mcp-Session-Id`; a 2026-07-28-conforming path has no reason to echo it, and GET/DELETE to the MCP endpoint should no longer be served (the spec removed the GET stream and specifies `405 Method Not Allowed` at **SHOULD** level, not MUST). This is the test that would settle the demoted inference in §7 — we have not run it.
