...
Five divergences, each read from source at a pinned commit on 2026-07-29. These are the questions to put to a vendor, because none of them appears in a feature matrix.
...
- **(A) Pre-spend reservation or deduction — safe.** LiteLLM (`reserve_budget_for_request` → `reconcile_budget_reservation`, with `fail_closed_budget_enforcement` to 503 when the reservation can't be written, and an explicitly reasoned cancel policy: reconcile to input-token cost rather than refunding to zero, *"so a caller [can't] abort pre-token to dodge that charge"*). new-api (`PreConsumeBilling` → deferred `Refund` → `SettleBilling(delta)`) — **except when the trust bypass fires**: `preConsume` opens with `if s.shouldTrust(c) { s.trusted = true; effectiveQuota = 0 … }`, so wallet accounts above the configured trust quota get no pre-deduction at all and degrade to class (B). See [chapter 7](virtual-keys-metering.md) §3.3.
