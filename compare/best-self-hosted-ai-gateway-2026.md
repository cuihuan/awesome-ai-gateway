...
> 📊 **Key numbers** · Self-hosting adds **$0** per-token markup — but the model still dominates the bill: a 100K-token report ranges **$0.03–$3.01** (a **~106×** spread, [computed](../BENCHMARKS.md#part-3--real-world-token-cost-computed), 2026-06). And star count ≠ safety — **LiteLLM** (<!--s:BerriAI/litellm-->⭐ 57.3k<!--/s-->) shipped 2026 CVEs, so weigh [patch discipline](../BENCHMARKS.md#part-4--gateway-scorecard-compliance--price--security--stability--observability), not just popularity. Self-hosting the gateway doesn't exempt you from provider outages: peer-reviewed measurements show the OpenAI/Anthropic APIs fail **~every 2 days** with ~1h median recovery ([ICPE 2025](https://arxiv.org/abs/2501.12469)) — cross-provider fallback in your gateway config is the fix.
...
|---|---|---|---|---|---|
| **LiteLLM** | Python | Breadth (100+ providers), features | 5.83 ms | MIT | <!--s:BerriAI/litellm-->⭐ 57.3k<!--/s--> |
