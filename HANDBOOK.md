...
| 7 | [Virtual keys, budgets & metering](docs/virtual-keys-metering.md) | ✅ **live** (2026-07-29) | What a virtual key actually scopes across gateways; pre-spend reservation vs post-hoc metering under concurrency; how streamed, reasoning and cached tokens get counted (and mis-counted); what spend is lost when the process dies; and the billing bugs that reached production |
| 8 | [MCP & agent gateways](docs/mcp-agent-gateways.md) | ✅ **live** (2026-07-29) | Six properties of a request that change when the caller is an agent; the 2026-07-28 stateless MCP rewrite and what it does to a gateway; tool-level authorization and secret brokering; the DNS-rebinding CVE class that hit five codebases; what the listed MCP gateways actually implement |

Reading order: 1 → 4 → 5 → 6 pick up the core; 2, 3, 7, 8 stand alone. **All eight chapters are live as of 2026-07-29.** The freshest evidence still lives in the list; the freshest evidence always lives in the [list](README.md) and [BENCHMARKS](BENCHMARKS.md) — chapters link into both rather than duplicating them.
